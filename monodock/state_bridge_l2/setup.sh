...
katana_1="katana-1" # Katana account

# Declare Class Hash 
class_hash=$(starkli declare "$json_file" --compiler-version "$compiler_version" --account "$katana_0")

# Deploy Class Contract
contract_address=$(starkli deploy "$class_hash" "$l1_address" "$tree_depth" --salt "$salt" --rpc "$katana_rpc" --account "$katana_1")
