...
impl Felt252VecImpl<T, +Drop<T>, +Copy<T>, +Felt252DictValue<T>> of VecTrait<Felt252Vec<T>, T> {
    ...
    fn pop(ref self: Felt252Vec<T>) -> Option<T> {
        ...
        self.len -= 1;
        Option::Some(self.items.get(self.len.into()))
    }
    ...
    fn is_empty(self: @Felt252Vec<T>) -> bool {
        *self.len == 0
    }
}
...
impl NullableVecImpl<T, +Drop<T>, +Copy<T>> of VecTrait<NullableVec<T>, T> {
    ...
    fn pop(ref self: NullableVec<T>) -> Option<T> {
        if self.is_empty() {
            return Option::None;
        }
        self.len -= 1;
        Option::Some(self.items.get(self.len.into()).deref())
    }
    ...
    fn is_empty(self: @NullableVec<T>) -> bool {
        *self.len == 0
    }
}

fn create_zero_values_dict(n: usize) -> (Felt252Vec<u128>, Felt252Vec<u128>) {
