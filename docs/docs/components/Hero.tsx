...
export function WorldIDHero() {
  ...
  // Content overflow detection - more aggressive for zoom
  useEffect(() => {
    const checkContentOverflow = () => {
      // More aggressive detection for zoom levels
      const isMobileWidth = window.innerWidth < 640;
      const constrainedHeight = window.innerHeight < 800; // Increased from 700
      
      // Enable scroll when needed
      const needsScroll = isMobileWidth || constrainedHeight;
    ...
    // Add a slight delay to re-check after zoom changes
  ...
  useEffect(() => {
    // Force recalculation with a small delay to handle zoom changes
    const applyStyles = () => {
      const isMobile = window.innerWidth <= 640;
      const isSmallHeight = window.innerHeight < 800; // Increased threshold
    ...
    // Also apply after a short delay to handle zoom transitions
  ...
  useEffect(() => {
    ...
    const heroContainer = document.getElementById('world-id-hero-root');
    if (heroContainer) {
      heroContainer.addEventListener('scroll', handleScroll);
      
      return () => {
        heroContainer.removeEventListener('scroll', handleScroll);
        clearTimeout(scrollTimeout);
      };
    }
  }, []);
  ...
  const isSmallHeight = windowHeight < 800; // Increased threshold
  const needsScroll = isMobile || contentOverflows || isSmallHeight;

  return (
    <>
      {/* Navigation Bar - Outside scroll container */}
      ...
      <div 
        ...
        style={{
          ...
          paddingTop: windowWidth < 640 ? '60px' : '80px' // Add padding for navbar
        }}
      >
          ...
          <div> {/* Wrap main content in a div */}
            ...
            <div style={{ marginBottom: '1.5rem' }}> {/* Reduced from 2rem */}
              <div
                ...
                onClick={() => window.open('https://github.com/NethermindEth/starknet-worldcoin-bridge/releases', '_blank')}
                onMouseEnter={(e) => {
                  const target = e.currentTarget as HTMLElement;
                  target.style.backgroundColor = 'rgba(255, 255, 255, 0.12)';
                  target.style.transform = 'translateY(-1px)';
                  target.style.boxShadow = '0 12px 40px rgba(0, 0, 0, 0.15), inset 0 1px 0 rgba(255, 255, 255, 0.15)';
                }}
                onMouseLeave={(e) => {
                  const target = e.currentTarget as HTMLElement;
                  target.style.backgroundColor = 'rgba(255, 255, 255, 0.08)';
                  target.style.transform = 'translateY(0px)';
                  target.style.boxShadow = '0 8px 32px rgba(0, 0, 0, 0.1), inset 0 1px 0 rgba(255, 255, 255, 0.1)';
                }}
              >
                <span style={{
                  borderRadius: '9999px',
                ...
                <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <span style={{ color: 'rgba(255, 255, 255, 0.9)', fontSize: '14px', fontWeight: '500' }}>Live on Sepolia Testnet</span>
...
pub trait IStarkWorldID<TContractState> {
    ...
    fn transfer_ownership(ref self: TContractState, from_address: felt252, new_owner: EthAddress);
          ...
          {/* Partners section */}
          <div style={{
            padding: windowWidth < 480 ? '0.4rem 0.5rem 1.5rem 0.5rem' : '0.5rem 1rem 2rem 1rem', // Increased bottom padding for breathing room
      ...
      {/* Add responsive styles with zoom detection */}
      <style>{`
        
        /* Custom Scrollbar Styling */
        #world-id-hero-root {
          scrollbar-width: thin;
          scrollbar-color: ${isScrolling ? 'rgba(255, 115, 22, 0.8) transparent' : 'transparent transparent'};
        }
        
        /* Webkit browsers (Chrome, Safari, Edge) */
        #world-id-hero-root::-webkit-scrollbar {
          width: 12px;
        }
        
        #world-id-hero-root::-webkit-scrollbar-track {
          background: transparent;
          border-radius: 10px;
        }
        
        #world-id-hero-root::-webkit-scrollbar-thumb {
          background: ${isScrolling ? 
            'linear-gradient(45deg, #ff4901, #ff6b35)' : 
            'transparent'
          };
          border-radius: 10px;
        ...
        #world-id-hero-root::-webkit-scrollbar-thumb:hover {
          background: ${isScrolling ? 
            'linear-gradient(45deg, #ea580c, #f97316)' : 
            'transparent'
          };
          box-shadow: 0 0 10px rgba(255, 73, 1, 0.4);
        }
        
        #world-id-hero-root::-webkit-scrollbar-corner {
          background: transparent;
        }
        ...
        /* High zoom detection - more aggressive */
